...
func ConvertValue(fd *model.FieldDescriptor, value string, fileD *model.FileDescriptor, node *model.Node) (ret string, ok bool) {
	...
	case model.FieldType_Enum:
		if fd.Complex == nil {
			glog.Errorf("%s, '%s'", "值转换: 枚举类型空", fd.Name)
			return "", false
		}

		evd := fd.Complex.FieldByValueAndMeta(value)
		if evd == nil {
			glog.Errorf("%s, '%s' '%s'", "值转换: 枚举值未找到", value, fd.Complex.Name)
			return "", false
		}
	...
	case model.FieldType_Struct:
		...
		if fd.Complex == nil {
			glog.Errorf("%s, '%s'", "值转换: 结构类型空", fd.Name)
			return "", false
		}
	...
	default:
		glog.Errorf("%s, '%s' '%s'", "值转换: 未知的字段类型", fd.Name, fd.Name)
		return "", false
	}
